...
view: class_weekly_stats {
  ...
  measure: max_total_assignments_to_date {
    description: "total number of assignments available to date"
    group_label: "Raw Metrics"
    type: max
  ...
  measure: max_assignments_worked_on_to_date {
    description: "total number of assignments worked on to date"
    group_label: "Raw Metrics"
    type: max
  ...
  measure: sum_started_after_due {
    description: "number of assignments started after due"
    group_label: "Raw Metrics"
    type: sum
    sql: ${TABLE}.started_after_due ;;
    value_format_name: decimal_0
  }
  measure: sum_missed_assignments {
    description: "number of assignments with no response"
    group_label: "Raw Metrics"
    type: sum
    sql: ${TABLE}.missed_assignments ;;
    value_format_name: decimal_0
  }
  measure: sum_late_submissions {
  ...
  measure: average_late_submissions {
    description: "avg late submissions per student"
    group_label: "Calculations"
    type: number
    sql: ${sum_late_submissions} / ${sum_student_count} ;;
    value_format_name: decimal_1
  }
  measure: average_missed_assignments {
    description: "avg missed assignments per student"
    group_label: "Calculations"
    type: number
    sql: ${sum_missed_assignments} / ${sum_student_count} ;;
    value_format_name: decimal_1
  }
}
...
view: student_assignment_stats {
  derived_table: {
    explore_source: sections {
      ...
      column: user_id {field: users.user_id}
      column: assignment_id {field: dim_assignment.assignment_id}
      derived_column: pk {sql: HASH(section_id,'|',user_id,'|',assignment_id);; }
      column: new_assignment_count {field: responses_final.new_assignment_count}
      column: assignment_duration {field:assignment_final.total_assignment_duration}
      column: assignment_submissions {field:responses.response_submission_count}
      column: assignment_time_on_task {field:responses.total_attempt_time}
...
view: student_question_stats {
  derived_table: {
    explore_source: sections {
      ...
      column: user_id {field: users.user_id}
      column: question_id {field: dim_question.question_id}
      derived_column: pk {sql: HASH(section_id,'|',user_id,'|',question_id);; }
      column: new_assignment_count {field: responses_final.new_assignment_count}
      column: assignment_time_on_task {field:responses.total_attempt_time}
      column: questions_correct {field:responses.response_question_correct_count}
      column: questions_answered {field:responses.response_question_answered_count}
